...
The OrdexGold Core daemon can be configured to act as a trusted "border
router", implementing the ordexgold wire protocol and relay, making
...
The ZeroMQ feature in OrdexGold Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building ordexgoldd:
...
For instance:
    ...
    $ ordexgoldd -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/ordexgoldd.tx.raw \
...
These options can also be provided in ordexgold.conf.
...
From the perspective of ordexgoldd, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into ordexgoldd directly. Furthermore, no information is
...
using. OrdexGoldd appends an up-counting sequence number to each
