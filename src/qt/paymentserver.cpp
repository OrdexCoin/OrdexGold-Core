...
const QString BITCOIN_IPC_PREFIX("ordexgold:");
    ...
    if (startLocalServer)
    {
        ...
        if (!uriServer->listen(name)) {
            ...
            QMessageBox::critical(nullptr, tr("Payment request error"),
                tr("Cannot start ordexgold: click-to-pay handler"));
        }
        else {
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith("ordexgold://", Qt::CaseInsensitive))
    {
        Q_EMIT message(tr("URI handling"), tr("'ordexgold://' is not a valid URI. Use 'ordexgold:' instead."),
    ...
    else if (s.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // bitcoin: URI
    {
        ...
        // normal URI
        {
            ...
            if (GUIUtil::parseBitcoinURI(s, &recipient))
            {
                ...
                Q_EMIT message(tr("URI handling"),
                    tr("URI cannot be parsed! This can be caused by an invalid OrdexGold address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
