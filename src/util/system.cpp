...
const char * const BITCOIN_CONF_FILENAME = "ordexgold.conf";
...
#else
    const char* pszModule = "ordexgold";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "OrdexGold";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/OrdexGold";
#else
    // Unix-like
    return pathRet / ".ordexgold";
#endif
#endif
}
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    if (copyright_devs.find("Bitcoin Core") == std::string::npos) {
            ...
            strYear.replace(strYear.find("2026"), sizeof("2026")-1, "2011-2026");
            strCopyrightHolders += "\n" + strYear + "The Litecoin Core developers";
            strYear.replace(strYear.find("2011-2026"), sizeof("2011-2026")-1, "2009-2026");
